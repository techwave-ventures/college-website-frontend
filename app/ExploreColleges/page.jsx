// app/ExploreColleges/page.jsx (or your path)
"use client";

import React, { useState, useEffect, useMemo } from "react";
import axios from "axios";
import { useRouter } from "next/navigation"; // Not strictly needed if Link is used primarily
...
import { Label } from "@/components/ui/label"; // Label might be used for future filters
...
function CollegeCard({ college }) {
  // --- MODIFIED: Use college.slug for navigation, with fallback to _id ---
  // Ensure your college objects from the API include a 'slug' field.
  // The link will be /college/your-college-slug
  const collegeLink = college.slug
    ? `/college/${college.slug}`
    : (college._id ? `/college/id/${college._id}` : "#"); // Fallback to an ID-based path or '#'
...
export default function ExploreColleges() {
  ...
  }, [selectedExamId]); // Added API_BASE_URL to dependencies if it could change, though unlikely for this setup
  ...
  const handleExamClick = (examId) => {
    ...
    setSearchTerm("");
    setShowFilters(false); // Close mobile filter on exam selection
  };
            ...
            <main className="flex-grow lg:w-3/4 xl:w-4/5">
              ...
              <div className="relative mb-6">
                <Input
                  ...
                  disabled={!selectedExamId || isLoadingColleges}
                />
